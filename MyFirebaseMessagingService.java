...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        if ("sms_updated".equals(type)) {
            ...
            // 2) 앱 종료 상태에서도 위젯 갱신 - Drive에서 최신 잔액 재읽기
                    ...
                    public void onSuccess(String content) {
                        ...
                        for (int i = blocks.length - 1; i >= 0; i--) {
                            ...
                            for (int j = 0; j < accounts.length; j++) {
                                ...
                                if (m.find()) {
                                    latestBal[j] = m.group(1) + "원";
                                    Log.d(TAG, "삭제후 파싱: " + accounts[j] + " = " + latestBal[j]);
                                }
                            }
    ...
    /** data-only 메시지이므로 직접 알림 표시 (잠금화면에 잔액 포함) */
    ...
    /** FCM 수신 시 Drive에 수신 기록 저장 */
    /** PinActivity에서 직접 호출 가능한 public 래퍼 */
    public static void saveFcmReceivedLogPublic(android.content.Context context) {
        new Thread(() -> {
            try {
                ...
                reader.readFile("fcm_received.txt", new DriveReadHelper.ReadCallback() {
                    @Override public void onSuccess(String existing) {
                        ...
                        new Thread(() -> {
                            try {
                                ...
                                up.uploadFileSync(sb.toString().trim(), "fcm_received.txt");
                            } catch (Exception e) {
                                android.util.Log.e("FCM", "수신기록 업로드 실패: " + e.getMessage());
                            }
                        }).start();
                    }
                    @Override public void onFailure(String error) {
                        new Thread(() -> {
                            try {
                                DriveUploadHelper up = new DriveUploadHelper(context);
                                up.uploadFileSync(newLine, "fcm_received.txt");
                            } catch (Exception e) {
                                android.util.Log.e("FCM", "수신기록 업로드 실패: " + e.getMessage());
                            }
                        }).start();
                ...
                android.util.Log.e("FCM", "saveFcmReceivedLogPublic 오류: " + e.getMessage());
            }
        }).start();
    }
        ...
        new Thread(() -> {
            try {
                ...
                reader.readFile("fcm_received.txt", new DriveReadHelper.ReadCallback() {
                    ...
                    @Override
                    public void onFailure(String error) {
                        uploadFcmLog(newLine);
                    }
                });
            } catch (Exception e) {
                Log.e(TAG, "FCM 수신 기록 오류: " + e.getMessage());
            }
        }).start();
    }
    ...
    private void uploadFcmLog(String content) {
        new Thread(() -> {
            try {
                ...
                up.uploadFileSync(content, "fcm_received.txt");
                Log.d(TAG, "FCM 수신 기록 저장 완료");
            } catch (Exception e) {
                Log.e(TAG, "FCM 수신 기록 업로드 실패: " + e.getMessage());
            }
        }).start();
    }
